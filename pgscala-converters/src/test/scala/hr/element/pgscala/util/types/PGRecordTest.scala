...
class PGRecordTest extends FeatureSpec
  ...
  feature("record") {
    scenario("can create simple record") {
      val record = "(\"NULL\",,\"\")"
    ...
    scenario("can unpack date") {
      ...
      date must be { new org.joda.time.LocalDate(2011, 02, 03) }
    }

    scenario("can unpack date time") {
      ...
      time.get must be { new org.joda.time.DateTime(2012, 01, 02, 20, 56, 15, 17) }
      number must be { 3 }
    }
  }
}
